...
$results = query("SELECT * FROM td_surat_masuk WHERE jenis = 'media partner' AND id LIKE '%$id%' AND YEAR(tanggal) LIKE '%$year%' ORDER BY id DESC");
...
<body class="bg-light">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="d-block justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
                    <div class="bg-opacity-75 rounded-4 my-3">
                        <p class=" fw-normal mb-0"><span class="opacity-50">Pages</span> / Media Partner</p>
                                        ...
                                        <tbody>
                                            <?php foreach ($results as $result) : ?>
                                                <tr class="text-muted fw-semibold align-middle">
                                                    <td class="border-0 fs-6 py-3"><a class="text-decoration-none">#MP<?= $result['id'] ?></a></td>
                                                    <td class="border-0 fs-6 py-3"><?= date_formatter($result['tanggal'], 'dd/mm/yyyy') ?></td>
                                                    <td class="border-0 fs-6 py-3"><?= $result['nama'] ?></td>
                                                    <td class="border-0 fs-6 py-3">Media Partner</td>
                                                    <td class="border-0 fs-6 py-3">
                                                        <a href="<?= $BASE_URL ?>app/data/file/media-partner/<?= $result['file'] ?>" download="<?= $result['nama'] . ' - ' . $result['judul'] ?>" class="btn btn-white fw-semibold">PDF</a>
                                                    </td>
                                                    <td class="border-0 fs-6 py-3">
                                                        <button onclick="window.location.href = '<?= $BASE_URL ?>admin/media-partner/edit?id=<?= $result['slug'] ?>'" class="btn btn-outline-primary bg-primary bg-opacity-25 border-0 px-lg-3 rounded-pill btn-sm my-1"><i class="bi bi-vector-pen"></i>&nbsp;Edit</bu>
                                                        ...
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <button type="button" class="btn btn-primary" onclick="window.location.replace('<?= $BASE_URL ?>admin/media-partner/delete?id=<?= $result['id'] ?>')">Delete</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    <script>
        document.getElementById('icon-media-partner').classList.add('bg-primary', 'text-white');
    </script>
